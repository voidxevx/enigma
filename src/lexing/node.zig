...
pub const INode = struct {
    ...
    pub const NodeVTable = struct {
        ...
        format: ?*const fn (ptr: *const anyopaque, writer: *std.io.Writer) error{WriteFailed}!void = null,
    };

    /// Format function for the node.
    /// 
    /// Allows the node to be printed using "{f}" in a formatted string.
    pub fn format(self: *const INode, writer: *std.io.Writer) !void {
...
pub const NullNode = struct {
    ...
    pub fn format(_: *const anyopaque, writer: *std.io.Writer) error{WriteFailed}!void {
